                  ...
                  @foreach($categories as $category)
                    <tr>
                      ...
                      <td style="white-space: nowrap;">
                        <a class="btn btn-info" title="Edit" href="{{ route('categories.edit', $category->id) }}">
                          <i class="fas fa-edit"></i>
                        </a>
                        <a class="btn btn-danger" title="Delete" href="{{ route('categories.destroy', $category->id) }}" 
                          onclick="event.preventDefault();document.getElementById('delete_{{$category->id}}').submit();">
                          <i class="fas fa-trash"></i>
                        </a>
                        <form class="d-none" method="post" action="{{ route('categories.destroy', $category->id) }}" id="delete_{{$category->id}}">
                          @csrf
                          @method('DELETE')
                        </form>
                      </td>
