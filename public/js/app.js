...
window.Toast = Toast;
window.Fire = new Vue();
Vue.filter("imgSrcUrl", function (img) {
  return "/frontsite/assets/img/products/" + img;
});
Vue.filter("modalCategoryUrl", function (id) {
  return "/collections/" + id;
});
Vue.filter("modalBrandUrl", function (id) {
  return "/brands/" + id;
});
Vue.filter("productUrl", function (item) {
  return "/product/".concat(item.id, "/details");
});
var app = new Vue({
  el: "#app",
  data: {
  ...
  methods: {
    getCart: function getCart() {
      ...
      axios.get("/cart-info").then(function (res) {
        _this2.cart = res.data;
      });
    },
    addItemToCart: function addItemToCart(id) {
      axios.post("/cart/".concat(id, "/add"), {
        quantity: 1
      }).then(function (res) {
        Fire.$emit("loadCart");
        Toast.fire({
    ...
    removeItemFromCart: function removeItemFromCart(item) {
      axios["delete"]("/cart/" + item.id).then(function (res) {
    ...
    showProductModal: function showProductModal(product) {
    ...
    productFormSubmit: function productFormSubmit(id) {
      var _this3 = this;
    ...
    increaseCart: function increaseCart(item) {
      axios.put("/cart-increase/" + item.id).then(function (res) {
        Fire.$emit("loadCart");
      });
    },
    decriseCart: function decriseCart(item) {
      if (item.quantity <= 1) {
        Toast.fire({
          icon: "info",
      ...
      axios.put("/cart-decrese/" + item.id).then(function (res) {
        Fire.$emit("loadCart");
      });
    }
  }
...
module.exports = __webpack_require__(/*! C:\xampp\htdocs\professional\hastech\resources\js\app.js */"./resources/js/app.js");
